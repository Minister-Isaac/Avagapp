    ...
    <div className="flex pt-5 px-3 flex-col gap-4">
      <div className="flex justify-between p-2 items-center text-white">
        ...
        <p
          ...
          className="flex text-sm justify-center w-full lg:w-[15%] cursor-pointer p-2 lg:p-[10px] items-center rounded-2xl gap-2 bg-main-dark"
        >
          <RiAiGenerate /> Gerar tudo
        </p>
      </div>
      ...
      <Card className="h-full lg:overflow-hidden overflow-x-scroll   w-full  px-6">
        <table className="w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {currentData.map(
              ({ value, last_name, first_name, pun, img, medal, desemp, id }, index) => {
                ...
                return (
                  <tr key={id} className="hover:bg-gray-50">
                    <td className={classes}>
                      ...
                      >
                        {check && (
                          <Checkbox onClick={() => addSelected(valueId)} />
                        )}
                        {startIndex + index + 1}
                      </Typography>
                    </td>
