    ...
    async function fetchQuestions() {
      try {
        ...
        const fillQuestions = categories.flatMap(category =>
          category.questions.filter(q => q.question_type === "fill_in_the_blank")
        );
    ...
    return parts.map((part, i) => (
      ...
      <button
        className={`bg-blue-500 mt-10 text-white py-2 px-4 rounded ${
          userAnswer.length !== questions[currentQuestionIndex].correct_answer.length
            ? "opacity-50 cursor-not-allowed"
            : ""
        }`}
        onClick={handleSubmit}
        disabled={userAnswer.length !== questions[currentQuestionIndex].correct_answer.length}
      >
        Enviar Resposta
      </button>
